...
const temporaryRoot = await mkdtemp(join(tmpdir(), 'dsh-control-plane-profile-smoke-'))
...
function parseTrailingJsonArray(output) {
  ...
  throw new Error('Control Plane npm pack did not emit a trailing JSON array')
}

async function packControlPlane() {
  ...
  const manifest = parseTrailingJsonArray(result.stdout)
  const filename = manifest[0]?.filename
  assert.equal(typeof filename, 'string', 'Control Plane npm pack returned no filename')
...
try {
  ...
  await writeFile(join(repositoryRoot, 'package.json'), `${JSON.stringify({
    name: 'dsh-control-plane-profile-smoke-fixture',
    version: '1.0.0',
    private: true,
    type: 'module',
    scripts: {
  ...
  assert.match(dump.stdout, /providerVersion: 0\.1\.0-rc\.10/u)
